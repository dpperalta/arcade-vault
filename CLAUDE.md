...
- **`mobile-porter`** — recibe un juego **ya implementado** (`app/juego/<slug>/jugar/`) y garantiza que **se vea y se juegue bien en un teléfono en vertical** (navegador móvil, viewport pequeño, `pointer: coarse`), tomando la **SPEC 10** (controles táctiles) como estándar. Primero **valida** el layout portrait contra la convención de SPEC 10 (`useCoarsePointer` + `TouchGamepad` montado bajo el `.crt` solo si `coarse`, config `PAD` según la tabla del spec, y el bloque `@media (pointer: coarse)` de `.av-player` en `globals.css`) y, si algo se ve mal en móvil, lo **implementa/arregla** (CSS/layout responsivo dentro de ese media query, cableado del mando, corrección de `PAD`) **sin alterar jugabilidad, física, dificultad ni puntuación** y sin romper desktop (el mando no se monta en desktop). Verifica **por píxeles** con capturas en viewport de teléfono, guardadas en `.playwright-screenshots` con prefijo `mobile-<slug>-*.png`. Solo cubre las rutas de juego: **no** toca biblioteca, salón, home, auth ni fichas de catálogo, **no** optimiza landscape, y **no** toca catálogo (`GAMES`) ni Supabase. A diferencia de `skin-designer` (que garantiza skins) o `game-planer`/`game-jam` (que operan sobre juegos nuevos vía specs), `mobile-porter` opera sobre un juego existente para garantizar su responsividad móvil.

- **`game-performance-booster`** — recibe el **id/slug de un juego ya implementado** (`app/juego/<slug>/jugar/`) y garantiza que **corra fluido y sin memoria creciente**, en desktop y en teléfono, tomando la **SPEC 12** (`specs/12-rendimiento-frogger.md`) como estándar. Su orden es innegociable: **primero mide, después optimiza**. Primero **instrumenta** (crea `app/components/PerfOverlay.tsx` si falta y lo monta bajo `?fps=1`) y registra una **línea base** de FPS, frame-time p95 y heap; luego **valida** los dos frentes —el bucle de dibujo del `engine.ts` (cero asignaciones por frame: `emitState()` por escalares, claves de paleta precalculadas tipo `carBodyKey`, constantes de módulo tipo `FWD`/`SIGNS`, caché de `c.font`) y los re-renders de `page.tsx` (HUD por `ref` sin `gs` en estado, entrada de nombre no controlada, `React.memo` en `TouchGamepad` + `useCallback`)— y, si algo falla, lo **implementa** en commits separados por frente. Exige **paridad de píxeles absoluta**: **no** toca jugabilidad, física, dificultad, puntuación, efectos visuales (`shadowBlur`, glow), el CSS del CRT, el catálogo (`GAMES`) ni Supabase; las optimizaciones que romperían el look se reportan como recomendación, no se aplican. Verifica con la tabla antes/después y capturas en `.playwright-screenshots` con prefijo `perf-<slug>-*.png`. Un juego por invocación. A diferencia de `skin-designer` (apariencia/skins) y `mobile-porter` (layout móvil), `game-performance-booster` opera sobre un juego existente para garantizar su rendimiento sin cambiar un solo píxel.
